...
void CMainFrame::OnHelpWorkflow()
{
	static const wchar_t kHelp[] =
		...
		L"На шаге 1 выбирается крутящий момент из ряда ГОСТ (2.5…400 Н·м). Номер варианта задания (1…8) "
		L"пока не задаётся в окне сборки — его уточнит преподаватель; в документе остаётся сохранённое значение "
		L"(по умолчанию вариант 1: 4 луча, табл. 1). Полумуфты по-прежнему показывают номер варианта только для справки.\n\n"
		L"1. «Задание» → 1) Сборка: Мкр из списка, валы, табл. 21.3.1 — ОК.\n\n"
		L"2. Пункты 2) и 3): полумуфты — «Из ГОСТ» под вал.\n\n"
		L"3. Пункт 4): звёздочка — «Из ГОСТ».\n\n"
